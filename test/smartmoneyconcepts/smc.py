...
class smc:
    ...
    @classmethod
    def check_high_low(cls, ohlc):
        """
        Previous High Low
        This method returns the historical high and low up to each bar and checks if the current bar
        exceeds the highest high or falls below the lowest low of all previous bars in the period.
    
        Parameters:
        ohlc: DataFrame - OHLC data with datetime index
    
        Returns:
        DataFrame with columns:
            PreviousHigh: the highest high of all previous bars
            PreviousLow: the lowest low of all previous bars
            BrokenHigh: 1 if current bar's high exceeds the highest high of all previous bars, else 0
            BrokenLow: 1 if current bar's low falls below the lowest low of all previous bars, else 0
        """
        ...
        for i in range(1, len(ohlc)):
            ...
            # 計算整個時間段的歷史最高價和最低價
            previous_high[i] = previous_data['high'].max()
            previous_low[i] = previous_data['low'].min()
            
            # 檢查當前 K 棒是否突破整個時間段的歷史高低點
            current_high = ohlc['high'].iloc[i]
            current_low = ohlc['low'].iloc[i]
            
            # 比較當前K棒與所有先前K棒的最高高點和最低低點
            currently_broken_high = current_high > previous_data['high'].max()
            currently_broken_low = current_low < previous_data['low'].min()
            
            broken_high[i] = 1 if currently_broken_high else 0
            broken_low[i] = 1 if currently_broken_low else 0
        
        # 轉換為 Pandas Series，保留原始索引
        previous_high = pd.Series(previous_high, name="PreviousHigh", index=ohlc.index)
        previous_low = pd.Series(previous_low, name="PreviousLow", index=ohlc.index)
        broken_high = pd.Series(broken_high, name="BrokenHigh", index=ohlc.index)
        broken_low = pd.Series(broken_low, name="BrokenLow", index=ohlc.index)
